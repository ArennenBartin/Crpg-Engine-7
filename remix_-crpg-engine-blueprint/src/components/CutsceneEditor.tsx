...
import { CutsceneData, EventActionData } from "../schema/game";
...
const unsupportedActionTypes = ["play_sound", "start_combat", "custom"] as const;
...
const actionLabels: Record<string, string> = {
  ...
  play_sound: "Play Sound (unsupported)",
    ...
    <div className="flex flex-col md:flex-row h-full w-full relative">
      ...
      {showAIModal && (
        <AIGenerationModal
          ...
          onGenerate={(data) => {
            const cutscenes = Array.isArray(data) ? data : [data];
            updateGamePackage({ cutscenes: [...gamePackage.cutscenes, ...cutscenes] });
            if (cutscenes.length > 0) setSelectedId(cutscenes[0].id);
          }}
  ...
  return (
    <div className="pl-0 md:pl-10 grid grid-cols-1 md:grid-cols-2 gap-3">
      ...
      {action.type === "set_switch" && (
        <>
          <TextField label="Switch ID" value={action.switch_id || ""} onChange={(switch_id) => updateAction(index, { switch_id })} />
      ...
      {action.type === "adjust_faction_rep" && (
        <>
          <TextField label="Faction ID" value={action.faction_id || ""} onChange={(faction_id) => updateAction(index, { faction_id })} />
