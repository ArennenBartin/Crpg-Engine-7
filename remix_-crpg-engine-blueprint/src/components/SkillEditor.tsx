          ...
          {(gamePackage.abilities || []).map((i) => (
            ...
            >
              <div className="flex flex-col min-w-0">
                ...
                <span className="text-xs text-neutral-500 truncate">{i.element} / {i.targeting}</span>
              </div>
            </button>
          ))}
                 ...
                 {activeSkill.payloads.map((payload, idx) => (
                    <div key={idx} className="bg-neutral-950/50 border border-neutral-800 p-2 rounded-md space-y-2">
                       <div className="flex gap-2 items-center">
                         <select
                           ...
                           onChange={(e) => {
                              ...
                              newPayloads[idx] = { ...newPayloads[idx], type: e.target.value as any };
                              handleUpdate({ payloads: newPayloads });
                           }}
                         >
                           <option value="damage">Damage</option>
                           <option value="heal">Heal</option>
                           <option value="status" disabled={payload.type !== "status"}>Status (unsupported)</option>
                           <option value="summon" disabled={payload.type !== "summon"}>Summon (unsupported)</option>
                         </select>
                         <input
                           type="number"
                       ...
                       {(payload.type === "status" || payload.type === "summon" || payload.target_tags?.length) && (
                         <div className="rounded border border-amber-700/40 bg-amber-950/20 p-2 text-xs text-amber-200 space-y-2">
                           <div className="flex items-center gap-2">
                             <AlertTriangle className="w-3.5 h-3.5" />
                             This payload data is schema-only; combat currently ignores it.
                           </div>
                           {payload.type === "status" && (
                             <input
                               disabled
                               value={payload.status_effect || ""}
                               placeholder="status_effect"
