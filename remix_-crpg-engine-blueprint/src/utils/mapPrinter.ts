...
const TERRAIN_GLYPH = (objectId: string | undefined, walkable: boolean): string => {
  ...
  if (objectId.includes("grave_earth") || objectId.includes("mud")) return "%";
...
const PLACEMENT_GLYPH = (objectId: string): string => {
  ...
  if (objectId.includes("fence") || objectId.includes("cordon")) return "'";
  if (objectId.includes("bell_tower")) return "B";
  if (objectId.includes("church.front") || objectId === "obj_p_church_front") return "F";
  if (objectId.includes("rotunda")) return "O";
  if (objectId.includes("monolith")) return "M";
  if (objectId.includes("witness") || objectId.includes("bleeding")) return "W";
  if (objectId.includes("glass_figure")) return "g";
  if (objectId.includes("lych_gate")) return "L";
  if (objectId.includes("market_cross")) return "+";
  if (objectId.includes("gibbet")) return "Y";
  if (objectId.includes("arch")) return "n";
  if (objectId.includes("well")) return "w";
  if (objectId.includes("notice") || objectId.includes("placard")) return "?";
  ...
  if (objectId.includes("shrine_stone")) return "$";
  if (objectId.includes("votive_token")) return "*";
  if (objectId.includes("stairs")) return ">";
  if (objectId.includes("lantern") || objectId.includes("candles")) return ".";
  if (objectId.includes("headstone")) return "o";
  if (objectId.includes("tomb")) return "□";
  if (objectId.includes("grave_cross")) return "†";
...
export function printMapAscii(result: MapBuildResult, opts: PrintOpts = {}): string {
  ...
  if (legend) {
    out.push("");
    out.push("legend: ' turf  . cobble  _ road  = flag  : boards  % grave  / plot  ~ water");
    out.push("        # wall  + door  > stairs  | bars  w well  ? notice  ≡ bridge  — dock");
    out.push("        T oak Y yew · shrub  o headstone † cross  □ tomb  ψ reeds");
    out.push("        W witness  M monolith  F church front  B bell tower  O rotunda  L lych  $ shrine");
    out.push("        s stall  I inn  c crate b barrel v cart  H stack f furnace");
    out.push("        N npc  P spawn  X exit  C container  i item  g glass-fig  r roof");
  }
  return out.join("\n");
}
